...
# --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (API, ОТПРАВКА, КЛАВИАТУРЫ) ---
...
# --- ЛОГИКА ПОЛЬЗОВАТЕЛЕЙ, БАЛАНСА, АДМИНКИ (код из вашей рабочей версии) ---
...
# --- ОБНОВЛЕНИЕ ИНТЕРФЕЙСА (ОДНО СООБЩЕНИЕ) ---
...
# --- ОБРАБОТЧИКИ CALLBACK И АДМИНКИ ---
...
def confirm_broadcast(chat_id, user_id):
    ...
    def send_all():
        global broadcast_active, broadcast_step, broadcast_message  # <--- исправлено: global вместо nonlocal
        ok = 0
        err = 0
